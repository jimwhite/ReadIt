...
public class Pair extends Format {
	...
	public String format(Alignment alignment) {
		...
		for (int i = 0; i * SEQUENCE_WIDTH < length; i++) {
			...
			subsequence1 = new Token[line - i * SEQUENCE_WIDTH];
			subsequence2 = new Token[line - i * SEQUENCE_WIDTH];
			...
			buffer.append(BLANK);
			for (Token t : subsequence1) { buffer.append(t.toString()); buffer.append(' '); }
			buffer.append(BLANK);
			...
			buffer.append(BLANK);
            for (Token t : subsequence2) { buffer.append(t.toString()); buffer.append(' '); }
			buffer.append(BLANK);
