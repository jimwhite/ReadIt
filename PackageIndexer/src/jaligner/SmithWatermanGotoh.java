...
public final class SmithWatermanGotoh {
	 ...
	 */
	private static Cell construct(Sequence s1, Sequence s2, Comparator<Token> comparator,
		...
		for (int i = 1, k = n; i < m; i++, k += n) {
			...
			for (int j = 1, l = k + 1; j < n; j++, l++) {
				similarityScore = comparator.compare(a1[i - 1], a2[j - 1]);
    ...
    {
        if (a == b) return 10;
        if (Character.isWhitespace(a) && Character.isWhitespace(b)) return 9;
        if (Character.toLowerCase(a) == Character.toLowerCase(b)) return 8;
	...
	private static Alignment traceback(Sequence s1, Sequence s2, Comparator<Token> m,
        ...
        Token[] reversed1 = new Token[maxlen]; // reversed sequence #1
        Token[] reversed2 = new Token[maxlen]; // reversed sequence #2
	...
	/**
	 * Reverses an array of tokens
	 * 
	 * @param a
	 * @param len
	 * @return the input array of char reserved
	 */
	private static Token[] reverse(Token[] a, int len) {
        Token[] b = new Token[len];
		for (int i = len - 1, j = 0; i >= 0; i--, j++) {
			b[j] = a[i];
		}
		return b;
